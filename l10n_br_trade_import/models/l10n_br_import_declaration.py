...
class ImportDeclaration(models.Model):
    ...
    def _import_xml_declaration(self, xml_string):
        ...
        self.customs_clearance_location = root.find(
            "armazenamentoRecintoAduaneiroNome"
        ).text
        self.customs_clearance_state_id = self.env["res.country.state"].search(
            [("name", "=", root.find("cargaUrfEntradaNome").text)], limit=1
        )
        ...
        if self.intermediary_type in ["conta_ordem", "encomenda"]:
            ...
            self.third_party_partner_id = self.env["res.partner"].search(
                [("name", "=", third_party_partner_name)], limit=1
            )
        ...
        def _import_xml_declaration(self, xml_string):
            ...
            self.transportation_type = root.find("conhecimentoCargaTipoNome").text
            self.afrmm_value = 0.0  # Set default value for AFRMM
            self.intermediary_type = root.find(
                "caracterizacaoOperacaoDescricaoTipo"
            ).text
